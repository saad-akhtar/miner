...
basic(Config) ->
    ...
    meck:expect(blockchain_ledger_v1, find_poc, fun(_, _) ->
        PoC = blockchain_ledger_poc_v2:new(
            <<"SecretHash">>, <<"OnionKeyHash">>, <<"Challenger">>, BlockHash
        ),
        {ok, [PoC]}
    end),
    ...
    {Onion, _} = blockchain_poc_packet:build(
        OnionKey,
        1234,
        [
            {PubKey, Data1},
    ...
    meck:expect(miner_onion_server, send_receipt, fun(
        Data0,
        OnionCompactKey0,
        Origin,
        _Time,
    ...
    }} = gen_udp:recv(Sock, 0, 5000),

    Packet = longfi:serialize(
        <<0:128/integer-unsigned-little>>, longfi:new(monolithic, 0, 1, 0, Onion, #{})
    ),
    ok = gen_udp:send(
        Sock,
        "127.0.0.1",
    ...
    {ok, {
        {127, 0, 0, 1},
        5678,
        <<?PROTOCOL_2:8/integer-unsigned, _Token1:2/binary, ?PULL_RESP:8/integer-unsigned,
            PacketJSON1/binary>>
    }} = gen_udp:recv(Sock, 0, 5000),
    ...
    meck:expect(miner_onion_server, send_receipt, fun(
        ...
        ?assertEqual(radio, Origin),
        Passed =
            Data2 == Data0 andalso libp2p_crypto:pubkey_to_bin(OnionCompactKey) == OnionCompactKey0,
    ...
    {ok, {
        {127, 0, 0, 1},
        5678,
        <<?PROTOCOL_2:8/integer-unsigned, Token2:2/binary, ?PULL_RESP:8/integer-unsigned,
            PacketJSON2/binary>>
    }} = gen_udp:recv(Sock, 0, 5000),
    gen_udp:send(
        Sock,
        {127, 0, 0, 1},
        5678,
        <<?PROTOCOL_2:8/integer-unsigned, Token2:2/binary, ?PULL_ACK:8/integer-unsigned,
            16#deadbeef:64/integer>>
    ),
