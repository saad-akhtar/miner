                ...
                case POCVersion of
                    V when V >= 9 ->
                        blockchain_utils:min_rcv_sig(
                            blockchain_utils:free_space_path_loss(TxLocation, Location, Frequency)
                        );
                    V when V >= 8 ->
                        FreeSpacePathLoss =
                            ?TRANSMIT_POWER -
            ...
            case Distance > 32 of
                ...
                false ->
                    ...
                    gen_udp:send(
                        ...
                        {127, 0, 0, 1},
                        Port,
                        <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary,
                            ?PUSH_DATA:8/integer-unsigned, 16#deadbeef:64/integer,
...
handle_info(go, State) ->
    ...
    [
        gen_udp:send(
            State#state.udp_sock,
            {127, 0, 0, 1},
            Port,
            <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PULL_DATA:8/integer-unsigned,
                16#deadbeef:64/integer>>
        )
     || {Port, _} <- State#state.udp_ports
    ],
    {noreply, State};
handle_info(
    {udp, UDPSock, IP, SrcPort,
        <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PULL_RESP:8/integer-unsigned,
            JSON/binary>>},
    State = #state{udp_sock = UDPSock, udp_ports = Ports, poc_version = POCVersion}
) ->
    gen_udp:send(
        ...
        IP,
        SrcPort,
        <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?TX_ACK:8/integer-unsigned,
            16#deadbeef:64/integer>>
    ),
    ...
    lists:foreach(
        fun({Port, Location}) ->
            ...
            ToSend =
                case POCVersion of
                    V when V >= 9 ->
                        FSPL = blockchain_utils:min_rcv_sig(
                            blockchain_utils:free_space_path_loss(
                                OriginLocation, Location, Frequency
                            )
                        ),
...
handle_info(
    {udp, _UDPSock, _IP, _SrcPort,
        <<?PROTOCOL_2:8/integer-unsigned, _Token:2/binary, ?PUSH_ACK:8/integer-unsigned>>},
    State
) ->
    {noreply, State};
handle_info(
    {udp, _UDPSock, _IP, _SrcPort,
        <<?PROTOCOL_2:8/integer-unsigned, _Token:2/binary, ?PULL_ACK:8/integer-unsigned>>},
    State
) ->
    {noreply, State};
        ...
        begin
            ...
            gen_udp:send(
                ...
                <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PUSH_DATA:8/integer-unsigned,
                    16#deadbeef:64/integer, Data/binary>>
            )
        end
     || {Port, Loc} <- UDPPorts
    ].
...
do_send(ToSendRSSI, ToSendSNR, Distance, _OriginLocation, _Location, Token, Packet, UDPSock, Port) ->
    case Distance > 32 of
        ...
        false ->
            NewJSON = #{
                <<"rxpk">> => [
                    maps:merge(maps:without([<<"imme">>, <<"rfch">>, <<"powe">>], Packet), #{
                        <<"rssi">> => ToSendRSSI,
                        <<"lsnr">> => ToSendSNR,
                        <<"tmst">> => erlang:system_time(seconds)
                    })
            ...
            ct:pal("sending ~p from ~p to ~p -> ~p km RSSI ~p, SNR ~p", [
                NewJSON, _OriginLocation, _Location, Distance, ToSendRSSI, ToSendSNR
            ]),
            gen_udp:send(
                UDPSock,
                {127, 0, 0, 1},
                Port,
                <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PUSH_DATA:8/integer-unsigned,
                    16#deadbeef:64/integer, (jsx:encode(NewJSON))/binary>>
            )
    end.

approx_snr(FSPL) ->
    PotentialSNRs = maps:keys(
        maps:filter(
            fun(_SNR, {Low, High}) ->
