...
init_per_testcase(_TestCase, Config0) ->
    ...
    try
        ...
        {ok, DKGCompletedNodes} = miner_ct_utils:initial_dkg(
            Miners,
            InitialVars ++ InitialCoinbaseTxns ++ AddGwTxns,
            Addresses,
            NumConsensusMembers,
            Curve
        ),
    ...
    ok = lists:foreach(
        fun(Miner) ->
            ...
            lists:any(
                fun(H) ->
                    try
                        {ok, ElectionRewardBlock} = ct_rpc:call(Miner, blockchain, get_block, [
                            H, Chain0
                        ]),
                        Txns = ct_rpc:call(Miner, blockchain_block, transactions, [
                            ElectionRewardBlock
                        ]),
                        ?assertEqual(length(Txns), 2),
                        [First, Second] = Txns,
                        ?assertEqual(
                            blockchain_txn:type(Second), blockchain_txn_consensus_group_v1
                        ),
            ...
            case
                {
                    ...
                    lists:member(Miner, NonConsensusMiners)
                }
            of
                {true, _, true, _} ->
