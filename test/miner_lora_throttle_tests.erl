...
us915_dwell_time_test() ->
    ...
    ?assertEqual(
        false, miner_lora_throttle:can_send(S2, T0 + Period - HalfMax - 1, Ch1, HalfMax + 1)
    ),
    ?assertEqual(
        false, miner_lora_throttle:can_send(S2, T0 + Period - HalfMax - 2, Ch1, HalfMax + 1)
    ),
    ?assertEqual(
        false, miner_lora_throttle:can_send(S2, T0 + Period - HalfMax - 3, Ch1, HalfMax + 1)
    ),
