...
-define(AUSTINLOCS1, [
    631781084745290239, 631781089167934463, 631781054839691775, 631781050465723903
]).
-define(AUSTINLOCS2, [
    631781452049762303, 631781453390764543, 631781452924144639, 631781452838965759
]).
-define(LALOCS, [631236297173835263, 631236292179769855, 631236329165333503, 631236328049271807]).
-define(CNLOCS1, [
    %% spare-tortilla-raccoon
    631649369216118271,
...
poc_dist_v4_partitioned_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v4_partitioned_test, Config, maps:put(?poc_version, 4, CommonPOCVars)
    ).
...
poc_dist_v5_partitioned_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v5_partitioned_test, Config, maps:put(?poc_version, 5, CommonPOCVars)
    ).

poc_dist_v5_partitioned_lying_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v5_partitioned_lying_test, Config, maps:put(?poc_version, 5, CommonPOCVars)
    ).
...
poc_dist_v6_partitioned_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v6_partitioned_test, Config, maps:put(?poc_version, 6, CommonPOCVars)
    ).

poc_dist_v6_partitioned_lying_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v6_partitioned_lying_test, Config, maps:put(?poc_version, 6, CommonPOCVars)
    ).
...
poc_dist_v7_partitioned_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v7_partitioned_test, Config, maps:put(?poc_version, 7, CommonPOCVars)
    ).

poc_dist_v7_partitioned_lying_test(Config) ->
    CommonPOCVars = common_poc_vars(Config),
    run_dist_with_params(
        poc_dist_v7_partitioned_lying_test, Config, maps:put(?poc_version, 7, CommonPOCVars)
    ).
...
poc_dist_v8_partitioned_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v8),
    run_dist_with_params(
        poc_dist_v8_partitioned_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
poc_dist_v8_partitioned_lying_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v8),
    run_dist_with_params(
        poc_dist_v8_partitioned_lying_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
poc_dist_v10_partitioned_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v10),
    run_dist_with_params(
        poc_dist_v10_partitioned_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
poc_dist_v10_partitioned_lying_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v10),
    run_dist_with_params(
        poc_dist_v10_partitioned_lying_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
poc_dist_v11_partitioned_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v11),
    run_dist_with_params(
        poc_dist_v11_partitioned_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
poc_dist_v11_partitioned_lying_test(Config) ->
    ...
    ExtraVars = extra_vars(poc_v11),
    run_dist_with_params(
        poc_dist_v11_partitioned_lying_test, Config, maps:merge(CommonPOCVars, ExtraVars)
    ).
...
basic_test(Config) ->
    ...
    GenConsensusGroupTx = blockchain_txn_consensus_group_v1:new(
        [Addr || {Addr, _} <- ConsensusMembers], <<>>, 1, 0
    ),
    ...
    true = miner_ct_utils:wait_until(fun() ->
        case sys:get_state(Statem) of
            {requesting, _} ->
                true;
            _Other ->
                ct:pal("got other state ~p", [_Other]),
                false
        end
    end),
    ?assertEqual(requesting, erlang:element(6, erlang:element(2, sys:get_state(Statem)))),
...
restart_test(Config) ->
    ...
    {ok, Statem0} = miner_poc_statem:start_link(#{
        delay => 5,
        base_dir => BaseDir
    }),
    ...
    % Blockchain is = to Chain
    ?assertEqual(Chain, erlang:element(3, erlang:element(2, sys:get_state(Statem0)))),
    % State is requesting
    ?assertEqual(requesting, erlang:element(6, erlang:element(2, sys:get_state(Statem0)))),
    ...
    {ok, Statem1} = miner_poc_statem:start_link(#{
        delay => 5,
        base_dir => BaseDir
    }),
    ...
    ?assertEqual(receiving, erlang:element(6, erlang:element(2, sys:get_state(Statem1)))),
    % Get reponses
    ?assert(maps:size(erlang:element(11, erlang:element(2, sys:get_state(Statem1)))) > 0),
    ...
    ?assertEqual(receiving, erlang:element(1, sys:get_state(Statem1))),
    % Get receiving_timeout
    ?assertEqual(0, erlang:element(12, erlang:element(2, sys:get_state(Statem1)))),
    ok = miner_ct_utils:add_block(Chain, ConsensusMembers, []),
                ...
                V when V >= 10 ->
                    ...
                    true = miner_ct_utils:wait_until(
                        fun() ->
                            ...
                            C3 = check_non_empty_poc_rewards(
                ...
                V when V > 3 ->
                    true = miner_ct_utils:wait_until(
                        fun() ->
                            ...
                            C2 =
                                (check_eventual_path_growth(TestCase, Miners) orelse
                                    check_subsequent_path_growth(
                                        challenger_receipts_map(find_receipts(Miners))
                                    )),
                            %% Check there are some poc rewards
...
setup_dist_test(TestCase, Config, VarMap, Status) ->
    ...
    {ok, _FakeRadioPid} = miner_fake_radio_backplane:start_link(
        maps:get(?poc_version, VarMap),
        45000,
        lists:zip(RadioPorts, Locations),
        Status
    ),
...
gen_locations(poc_dist_v11_partitioned_lying_test, _, _) ->
    {
        ?AUSTINLOCS1 ++ ?LALOCS,
        lists:duplicate(4, hd(?AUSTINLOCS1)) ++ lists:duplicate(4, hd(?LALOCS))
    };
...
initialize_chain(Miners, TestCase, Config, VarMap) ->
    ...
    {ok, DKGCompletedNodes} = miner_ct_utils:initial_dkg(
        Miners, InitialTransactions, Addresses, N, Curve
    ),
...
challenger_receipts_map(Receipts) ->
    ReceiptMap = lists:foldl(
        fun({_Height, Receipt} = R, Acc) ->
            {ok, Challenger} = erl_angry_purple_tiger:animal_name(
                libp2p_crypto:bin_to_b58(blockchain_txn_poc_receipts_v1:challenger(Receipt))
            ),
...
request_counter(TotalRequests) ->
    lists:foldl(
        fun(Req, Acc) ->
            {ok, Challenger} = erl_angry_purple_tiger:animal_name(
                libp2p_crypto:bin_to_b58(blockchain_txn_poc_request_v1:challenger(Req))
            ),
            case maps:get(Challenger, Acc, undefined) of
                undefined ->
...
check_growing_paths(TestCase, ReceiptMap, ActiveGateways, PartitionFlag) ->
    Results = lists:foldl(
        fun({_Challenger, TaggedReceipts}, Acc) ->
            ...
            Res =
                length(blockchain_txn_poc_receipts_v1:path(FirstReceipt)) >= 1 andalso
                    RemainingGrowthCond,
            [Res | Acc]
        end,
        [],
    ...
    lists:all(fun(R) -> R == true end, Results) andalso
        maps:size(ReceiptMap) == maps:size(ActiveGateways).
...
check_remaining_partitioned_grow(TestCase, TaggedReceipts, ActiveGateways) ->
    Res = lists:map(
        fun({_, Receipt}) ->
            ...
            PathLength > 1 andalso PathLength =< 4 andalso
...
receipt_counter(ReceiptMap) ->
    lists:foldl(
        fun({Name, ReceiptList}, Acc) ->
            Counts = lists:map(
                fun({Height, ReceiptTxn}) ->
        ...
        fun() ->
            case maps:get(poc_version, VarMap, 1) of
                ...
                _ ->
                    ...
                    ct:pal(
                        "all can challenge: ~p, multiple requests: ~p, paths grow: ~p, rewards given: ~p",
                        [C1, C2, C3, C4]
                    ),
...
check_poc_rewards(RewardsTxns) ->
    ...
    RewardTypes = lists:foldl(
        fun(RewardTxn, Acc) ->
            Types = [
                blockchain_txn_reward_v1:type(R)
             || R <- blockchain_txn_rewards_v1:rewards(RewardTxn)
            ],
...
check_subsequent_path_growth(ReceiptMap) ->
    PathLengths = [
        length(blockchain_txn_poc_receipts_v1:path(Txn))
     || {_, Txn} <- lists:flatten(maps:values(ReceiptMap))
    ],
