...
init_per_testcase(_TestCase, Config0) ->
    ...
    try
        ...
        {ok, DKGCompletedNodes} = miner_ct_utils:initial_dkg(
            Miners,
            InitialVars ++ InitialPaymentTransactions ++ InitialDCTransactions ++
                InitialGenGatewayTxns,
            Addresses,
            NumConsensusMembers,
            Curve
...
basic(Config) ->
    ...
    Txn = ct_rpc:call(Owner, blockchain_txn_oui_v1, new, [
        OUI, OwnerPubKeyBin, [RouterPubkey], Filter, 8
    ]),
    ...
    receive
        {sc_handler, Thing, HandlerPid} ->
            ...
            ?assertEqual(
                {eui, DevEUI, AppEUI}, blockchain_helium_packet_v1:routing_info(HeliumPacket)
            ),
    ...
    lists:foreach(
        fun(Miner) ->
            ct_rpc:call(Miner, application, set_env, [
                miner, default_routers, [libp2p_crypto:pubkey_bin_to_p2p(RouterPubkey)]
            ])
        end,
        Miners
    ),
