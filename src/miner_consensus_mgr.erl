...
-spec election_done(
    binary(),
    [{libp2p_crypto:pubkey_bin(), binary()}],
    [libp2p_crypto:address()],
...
election_done(Artifact, Signatures, Members, PrivKey, Height, Delay) ->
    gen_server:cast(
        ?MODULE, {election_done, Artifact, Signatures, Members, PrivKey, Height, Delay}
    ).

rescue_done(Artifact, Signatures, Members, PrivKey, Height, Delay) ->
    gen_server:call(
        ?MODULE, {rescue_done, Artifact, Signatures, Members, PrivKey, Height, Delay}, infinity
    ).
...
handle_call(
    {genesis_block_done, BinaryGenesisBlock, Signatures, Members, PrivKey, _Height, _Delay},
    _From,
    State
) ->
    ...
    {ok, Group} = libp2p_swarm:add_group(
...
handle_call(
    {rescue_done, _Artifact, _Signatures, Members, PrivKey, _Height, _Delay},
    _From,
...
handle_call({initial_dkg, GenesisTransactions, Addrs, N, Curve}, From, State0) ->
    ...
    case do_initial_dkg(GenesisTransactions, Addrs, N, Curve, State) of
        {true, DKGState} ->
            lager:info("Waiting for DKG, From: ~p, WorkerAddr: ~p", [
                From, blockchain_swarm:pubkey_bin()
            ]),
            {noreply, DKGState#state{dkg_await = From}};
        {false, NonDKGState} ->
            lager:info("Not running DKG, From: ~p, WorkerAddr: ~p", [
                From, blockchain_swarm:pubkey_bin()
            ]),
...
handle_cast(
    {election_done, _Artifact, Signatures, Members, PrivKey, Height, Delay},
    State = #state{
        chain = Chain,
        current_dkgs = DKGs,
            ...
            ok = blockchain_txn_mgr:submit(
                ...
                fun(Res) ->
                    case Res of
                        ok ->
                            lager:info(
                                "Election successful, Height: ~p, Delay: ~p, Members: ~p, Proof: ~p",
                        ...
                        {error, Reason} ->
                            lager:error(
                                "Election failed, Height: ~p, Members: ~p, Proof: ~p, Delay: ~p, Reason: ~p",
...
->
    ...
    case blockchain:get_block(Hash, State#state.chain) of
        {ok, Block} ->
            ...
            lager:debug("got block at ~p sync ~p T ~p", [
                BlockHeight, Sync, (Now - blockchain_block:time(Block))
            ]),
            case Sync andalso (Now - blockchain_block:time(Block) > 3600) of
                ...
                false ->
                    ...
                    State2 =
                        ...
                        case
                            ElectionRunning andalso
                                (not maps:is_key({Height, Delay}, DKGs)) andalso
                                BlockHeight == Height + Delay
                        of
                            true ->
                                ...
                                case maps:find(EID, State#state.current_dkgs) of
                                    ...
                                    {ok, out} ->
                                        ...
                                        State2#state{
                                            ...
                                            started_groups = cleanup_groups(
                                                EID, State#state.started_groups, Ledger
                                            ),
                                    ...
                                    {ok, ElectionGroup} ->
                                        HBBFTGroup =
                                            case maps:find(EID, State#state.started_groups) of
                                                ...
                                                error ->
                                                    lager:info(
                                                        "starting hbbft ~p+~p at block height ~p",
                                        ...
                                        {ok, NewConsensusAddrs} = blockchain_ledger_v1:consensus_members(
                                            blockchain:ledger(State#state.chain)
                                        ),
                                        {ok, OldLedger} = blockchain:ledger_at(
                                            BlockHeight - 1, State#state.chain
                                        ),
                                        {ok, OldConsensusAddrs} = blockchain_ledger_v1:consensus_members(
                                            OldLedger
                                        ),
                                        ...
                                        lists:foreach(
                                            ...
                                            blockchain_utils:shuffle(
                                                NewConsensusAddrs -- OldConsensusAddrs
                                            )
                                        ),
                                        Round = blockchain_block:hbbft_round(Block),
                                        activate_hbbft(
                                            HBBFTGroup,
                                            State#state.active_group,
                                            State#state.ag_monitor,
                                            Round
                                        ),
                                        Ref = erlang:monitor(process, HBBFTGroup),
                                        State2#state{
                                            current_dkgs = maps:remove(
                                                EID,
                                                cleanup_groups(
                                                    EID, State2#state.current_dkgs, Ledger, true
                                                )
                                            ),
                                            started_groups = cleanup_groups(
                                                EID, State#state.started_groups, Ledger
...
handle_info(
    {blockchain_event, {integrate_genesis_block, _Hash}}, State = #state{chain = undefined}
) ->
    %% check if we're in the consensus group
    self() ! timeout,
    Chain = blockchain_worker:blockchain(),
...
handle_info(timeout, State) ->
    try
        {ok, #block_info_v2{height = StartHeight, hbbft_round = Round}} = blockchain:head_block_info(
            State#state.chain
        ),
            ...
            {ok, ConsensusAddrs} ->
                ...
                RestoreState =
                    case lists:member(blockchain_swarm:pubkey_bin(), ConsensusAddrs) of
                        true ->
                            ...
                            Pos = miner_util:index_of(
                                blockchain_swarm:pubkey_bin(), ConsensusAddrs
                            ),
                            Name = consensus_group_name(
                                ElectionHeight, ElectionDelay, ConsensusAddrs
                            ),
                            {ok, BatchSize} = blockchain:config(?batch_size, Ledger),
                            ...
                            case wait_for_group(Group) of
                                ...
                                {error, cannot_start} ->
                                    ...
                                    case BlockHeight < (ElectionHeight + ElectionDelay + 10) of
                                        true ->
                                            restore_dkg(
                                                ElectionHeight, ElectionDelay, Round, State
                                            );
                                        _ ->
                                            State
                                    end;
                ...
                case NextElection =< StartHeight of
                    true ->
                        ...
                        lager:info(
                            "try to start or restore the election group next ~p start ~p delay ~p",
                            [NextElection, StartHeight, Delay]
                        ),
                        State1 = RestoreState#state{
                            initial_height = NextElection,
                            delay = Delay
                        },
        ...
        C:E:S ->
            ...
            %% unrepairable state.
            lager:warning(
                "crash during restore process, going to idle state ~p:~p ~p",
                [C, E, S]
            ),
            {noreply, State}
    end;
handle_info(
    {'DOWN', MinerRef, process, _MinerPid, _Reason},
...
handle_info(
    {'DOWN', OldRef, process, _GroupPid, _Reason},
    #state{
        ag_monitor = OldRef,
        active_group = OldGroup,
        chain = Chain
    } = State
) ->
...
rescue_dkg(Members, Artifact, State) ->
    ...
    {_, State1} = do_dkg(
        ...
        Artifact,
        {?MODULE, sign_artifact},
        % I think length is OK only here
        rescue_done,
        length(Members),
        Curve,
        true,
...
restart_dkg(Height, Delay, State) ->
    ...
    case
        do_dkg(
            ...
            N,
            Curve,
            % do not create this if it doesn't exist
            false,
            % blank state woo
            #state{
                initial_height = Height,
    ...
    of
        ...
        {_, DKGState} ->
            ...
            case start_hbbft(DKGGroup, Height, Delay, Chain) of
                {ok, HBBFTGroup} ->
                    activate_hbbft(
                        HBBFTGroup, State#state.active_group, State#state.ag_monitor, Round
                    ),
                    Ref = erlang:monitor(process, HBBFTGroup),
                    {ok, State#state{
                        current_dkgs = maps:remove(
                            EID, cleanup_groups(EID, State#state.current_dkgs, Ledger, true)
                        ),
...
do_initial_dkg(GenesisTransactions, Addrs, N, Curve, State) ->
    ...
    GenesisBlockTransactions =
        GenesisTransactions ++
            [blockchain_txn_consensus_group_v1:new(ConsensusAddrs, <<>>, 1, 0)],
    Artifact = blockchain_block:serialize(
        ...
        true ->
            ...
            lists:foreach(
                fun(Member) ->
                    spawn(fun() ->
                        blockchain_fastforward_handler:dial(
                            Swarm, Chain, libp2p_crypto:pubkey_bin_to_p2p(Member)
                        )
                    end)
                end,
                blockchain_utils:shuffle(ConsensusAddrs -- [MyAddress])
            ),
            ...
            GroupArg = [
                ...
                [
                    ...
                    %% NOTE: F for DKG is 0
                    0,
                    %% NOTE: T for DKG is the byzantine F
                    F,
                    Artifact,
                    Sign,
                    {?MODULE, Done},
                    list_to_binary(DKGGroupName),
                    Height,
                    Delay
                ],
                [{create, Create}]
            ...
            {ok, DKGGroup} = libp2p_swarm:add_group(
                blockchain_swarm:tid(),
                DKGGroupName,
                libp2p_group_relcast,
                GroupArg
            ...
            lager:info(
                "height ~p delay ~p Address: ~p, ConsensusWorker pos: ~p, Group name ~p",
...
cleanup_groups({Start, _Delay} = EID, Groups, Ledger, Retain) ->
    ...
    maps:fold(
        ...
        fun
            (K, _V, G) when K == EID andalso Retain == false ->
