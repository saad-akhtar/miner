...
-record(state, {
    ...
    % last concentrator tmst reported by the packet forwarder
    last_tmst_us = undefined :: undefined | integer(),
    % last local monotonic timestamp taken when packet forwarder reported last tmst
    last_mono_us = undefined :: undefined | integer(),
...
send(
    ...
    gen_server:call(
        ?MODULE, {send, Payload, When, ChannelSelectorFun, DataRate, Power, true, Packet}, 11000
    ).

-spec send_poc(binary(), any(), function(), iolist(), any()) ->
    ok | {error, any()} | {warning, any()}.
send_poc(Payload, When, ChannelSelectorFun, DataRate, Power) ->
    gen_server:call(
        ?MODULE, {send, Payload, When, ChannelSelectorFun, DataRate, Power, false, undefined}, 11000
    ).
...
-spec reg_domain_data_for_addr(libp2p_crypto:pubkey_bin(), state()) ->
    {error, any()} | {ok, freq_data()}.
                                ...
                                {ok, Region} ->
                                    case blockchain_region_params_v1:for_region(Region, Ledger) of
                                        {ok, RegionParams} ->
                                            {ok,
                                                {Region, [
                                                    (blockchain_region_param_v1:channel_frequency(
                                                        RP
                                                    ) / ?MHzToHzMultiplier)
                                                 || RP <- RegionParams
                                                ]}};
...
handle_call(
    {send, _Payload, _When, _ChannelSelectorFun, _DataRate, _Power, _IPol, _HlmPacket},
    _From,
...
handle_call(
    {send, Payload, When, ChannelSelectorFun, DataRate, Power, IPol, HlmPacket}, From, State
) ->
    case
        send_packet(
            Payload, When, ChannelSelectorFun, DataRate, Power, IPol, HlmPacket, From, State
        )
    of
...
handle_call(position, _From, #state{pubkey_bin = Addr} = State) ->
    try
        ...
        C:E:S ->
            lager:warning(
                "error trying to get position: ~p:~p ~p",
                [C, E, S]
            ),
...
handle_info(
    reg_domain_timeout,
    #state{reg_domain_confirmed = false, pubkey_bin = Addr, chain = Chain} = State
) ->
    ...
    try
        ...
        _Type:Exception ->
            lager:warning(
                "error whilst checking regulatory domain: ~p.  chain: ~p. Will try again...", [
                    Exception, Chain
                ]
            ),
...
-spec handle_udp_packet(binary(), inet:ip_address(), inet:port_number(), integer(), state()) ->
    state().
handle_udp_packet(
    <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PUSH_DATA:8/integer-unsigned, MAC:64/integer,
        JSON/binary>>,
    IP,
    Port,
    RxInstantLocal_us,
...
handle_udp_packet(
    <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?TX_ACK:8/integer-unsigned, _MAC:64/integer,
        MaybeJSON/binary>>,
    _IP,
    _Port,
    _RxInstantLocal_us,
    #state{packet_timers = Timers, reg_throttle = Throttle} = State0
) ->
    case maps:find(Token, Timers) of
        %% empty string means success, at least with the semtech reference implementation
        {ok, {send, Ref, From, SentAt, LocalFreq, TimeOnAir, _HlmPacket}} when MaybeJSON == <<>> ->
            _ = erlang:cancel_timer(Ref),
            _ = gen_server:reply(From, ok),
            State0#state{
                packet_timers = maps:remove(Token, Timers),
                reg_throttle = miner_lora_throttle:track_sent(
                    Throttle, SentAt, LocalFreq, TimeOnAir
                )
            };
            ...
            {Reply, NewState} =
                case kvc:path([<<"txpk_ack">>, <<"error">>], jsx:decode(MaybeJSON)) of
                    <<"NONE">> ->
                        ...
                        Throttle1 = miner_lora_throttle:track_sent(
                            Throttle, SentAt, LocalFreq, TimeOnAir
                        ),
                    ...
                    <<"TOO_LATE">> ->
                        ...
                        case blockchain_helium_packet_v1:rx2_window(HlmPacket) of
                            undefined ->
                                lager:warning("No RX2 available"),
                                {{error, too_late}, State1};
                            _ ->
                                retry_with_rx2(HlmPacket, From, State1)
                        end;
                    <<"TOO_EARLY">> ->
                            ...
                            <<"TX_POWER">> ->
                                %% modified 1301 and unmodified 1302 will send this
                                {
                                    {warning,
                                        {tx_power_corrected,
                                            kvc:path(
                                                [<<"txpk_ack">>, <<"value">>], jsx:decode(MaybeJSON)
                                            )}},
...
handle_json_data(
    #{<<"stat">> := Status} = Map, Gateway0, RxInstantLocal_us, #state{gateways = Gateways} = State
) ->
...
handle_packets([], _Gateway, _RxInstantLocal_us, State) ->
    State;
handle_packets(
    _Packets, _Gateway, _RxInstantLocal_us, #state{reg_domain_confirmed = false} = State
) ->
    State;
handle_packets(
    [Packet | Tail], Gateway, RxInstantLocal_us, #state{reg_region = Region, chain = Chain} = State
) ->
            ...
            UseRSSIS =
                case
                    Chain /= undefined andalso
                        blockchain:config(?poc_version, blockchain:ledger(Chain))
                of
                    {ok, X} when X > 10 -> true;
                    _ -> false
                end,
    ...
    handle_packets(Tail, Gateway, RxInstantLocal_us, State#state{
        last_mono_us = RxInstantLocal_us, last_tmst_us = maps:get(<<"tmst">>, Packet)
    }).
...
route(Pkt) ->
    case longfi:deserialize(Pkt) of
        ...
        {ok, LongFiPkt} ->
            ...
            try
                longfi:type(LongFiPkt) == monolithic andalso longfi:oui(LongFiPkt) == 0 andalso
                    longfi:device_id(LongFiPkt) == 1
            of
                true ->
...
route_non_longfi(
    <<MType:3, _:5, DevAddr:32/integer-unsigned-little, _ADR:1, _ADRACKReq:1, _ACK:1, _RFU:1,
        FOptsLen:4, _FCnt:16/little-unsigned-integer, _FOpts:FOptsLen/binary, PayloadAndMIC/binary>>
) when
...
send_to_router(Type, RoutingInfo, Packet, Region) ->
    ...
    HeliumPacket = blockchain_helium_packet_v1:new(
        Type, Data, Time, RSSI, Freq, DataRate, SNR, RoutingInfo
    ),
    blockchain_state_channels_client:packet(
        HeliumPacket, application:get_env(miner, default_routers, []), Region
    ).

-spec country_code_for_addr(libp2p_crypto:pubkey_bin()) ->
    {ok, binary()} | {error, failed_to_find_geodata_for_addr}.
country_code_for_addr(Addr) ->
    B58Addr = libp2p_crypto:bin_to_b58(Addr),
    URL =
        application:get_env(miner, api_base_url, "https://api.helium.io/v1") ++ "/hotspots/" ++
            B58Addr,
    case httpc:request(get, {URL, []}, [{timeout, 5000}], []) of
        {ok, {{_HTTPVersion, 200, _RespBody}, _Headers, JSONBody}} = Resp ->
            ...
            CC = maps:get(
                <<"short_country">>, maps:get(<<"geocode">>, maps:get(<<"data">>, Body)), undefined
            ),
            {ok, CC};
        _ ->
    ...
    try file:read_line(F) of
        {ok, Line} ->
            ...
            Rec = #country{
                ...
                long = Long,
                name = Country,
                region = binary_to_atom(Region, utf8),
                geo_zone = binary_to_atom(GeoZone, utf8)
            },
...
%% at `Tmst_us`.
-spec tmst_to_local_monotonic_time(
    immediate | integer(), undefined | integer(), undefined | integer()
) -> integer().
...
packet_rssi(Packet, UseRSSIS) ->
    case maps:get(<<"rssi">>, Packet, undefined) of
        ...
        undefined ->
            ...
            Key =
                case UseRSSIS andalso maps:is_key(<<"rssis">>, Packet) of
                    true ->
                        <<"rssis">>;
                    _ ->
                        <<"rssic">>
...
send_packet(
    ...
    case select_gateway(Gateways) of
        ...
        {ok, #gateway{ip = IP, port = Port}} ->
            ...
            TimeOnAir = miner_lora_throttle:time_on_air(
                Bandwidth, SpreadingFactor, 5, 8, true, byte_size(Payload)
            ),
            ...
            {ok, State#state{
                packet_timers = maps:put(
                    Token, {send, Ref, From, SentAt, LocalFreq, TimeOnAir, HlmPacket}, Timers
                )
            }}
    end.
...
retry_with_rx2(HlmPacket0, From, State) ->
    #window_pb{
        ...
        frequency = Freq,
        datarate = DataRate
    } = blockchain_helium_packet_v1:rx2_window(HlmPacket0),
...
maybe_update_reg_data(#state{reg_domain_confirmed = true, chain = undefined} = State) ->
    ...
    State;
maybe_update_reg_data(#state{reg_domain_confirmed = true, chain = Chain} = State) when
    Chain /= undefined
->
    case application:get_env(miner, region_override, undefined) of
        ...
        Region ->
            ...
            case blockchain_region_params_v1:for_region(Region, blockchain:ledger(Chain)) of
                {ok, RegionParams} ->
                    FrequencyList = [
                        (blockchain_region_param_v1:channel_frequency(RP) / ?MHzToHzMultiplier)
                     || RP <- RegionParams
                    ],
