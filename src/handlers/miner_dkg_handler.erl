...
handle_command({stop, _Timeout}, #state{privkey = PKey, done_acked = false} = State) when
    PKey /= undefined
->
    {reply, {error, not_done}, [], State};
...
->
            ...
            case
                {
                    ...
                    libp2p_crypto:verify(
                        State#state.artifact, Signature, libp2p_crypto:bin_to_pubkey(Address)
                    )
                }
            of
                {true, true} ->
    ...
    case length(GoodSignatures) of
        ...
        SigLen when SigLen >= (2 * F) + 1 andalso State#state.sent_conf == false ->
            lager:debug("conf ~p/~p - ~p", [
                length(GoodSignatures), length(Members), State#state.signatures_required
            ]),
            {State#state{sent_conf = true}, [{multicast, t2b({conf, GoodSignatures})}]};
        _ ->
...
->
    ...
    case
        {
            lists:member(Address, Members) andalso
                %% not a duplicate
                not lists:keymember(Address, 1, Sigs),
            %% valid signature
            libp2p_crypto:verify(
                State#state.artifact, Signature, libp2p_crypto:bin_to_pubkey(Address)
            )
    ...
    of
        {true, true} ->
            lager:debug("got good sig ~p from ~p, have ~p", [
                Signature,
                Index,
            ...
            case length(NewState#state.signatures) of
                ...
                SigLen when SigLen == (2 * F) + 1 ->
                    lager:debug("conf ~p/~p - ~p", [
                        length(NewState#state.signatures),
                        length(Members),
                        State#state.signatures_required
                    ]),
                    {NewState#state{sent_conf = true}, [
                        {multicast, t2b({conf, NewState#state.signatures})}
                    ]};
                SigLen when SigLen == (2 * F) + 1 ->
                    lager:debug("sig echo ~p/~p - ~p", [
                        length(NewState#state.signatures),
                        length(Members),
                        State#state.signatures_required
                    ]),
...
deserialize(MapState0) when is_map(MapState0) ->
    ...
    PrivKey =
        case maps:get(privkey, MapState, undefined) of
            undefined ->
                undefined;
            Other ->
...
restore(OldState, NewState) ->
    lager:debug("called restore with ~p ~p", [
        OldState#state.privkey,
