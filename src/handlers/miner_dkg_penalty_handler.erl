...
-record(state, {
    bbas :: #{pos_integer() => hbbft_bba:bba_data()},
...
handle_command(start, State) ->
    {NewBBAs, OutMsgs} = lists:foldl(
        fun({BBAIndex, SawSig}, {BBAs, Msgs}) ->
            Input =
                case SawSig of
                    true -> 1;
                    false -> 0
        ...
        {State#state.bbas, []},
        lists:zip(lists:seq(1, length(State#state.members)), State#state.dkg_results)
    ),
...
handle_message({conf, Signatures}, _Index, State) when
    State#state.artifact /= undefined andalso State#state.done_called == false
->
    GoodSignatures =
        lists:foldl(
            fun({Address, Signature}, Acc) ->
                ...
                case
                    {
                        lists:keymember(Address, 1, Acc) == false andalso
                            lists:member(Address, State#state.members),
                        blockchain_txn_consensus_group_failure_v1:verify_signature(
                            State#state.artifact, Address, Signature
                        )
                    }
                of
                    {true, true} ->
    ...
    case length(GoodSignatures) of
        ...
        SigLen when SigLen >= Majority andalso State#state.conf_sent == false ->
            {State#state{signatures = GoodSignatures, conf_sent = true}, [
...
handle_message({conf, _Signatures}, _Index, _State) ->
    ignore;
handle_message({signature, Address, Signature}, _Index, State) when
    State#state.artifact /= undefined andalso State#state.done_called == false
->
    ...
    case
        {
            lists:member(Address, State#state.members) andalso
                not lists:keymember(Address, 1, State#state.signatures),
            blockchain_txn_consensus_group_failure_v1:verify_signature(
                State#state.artifact, Address, Signature
            )
        }
    of
        {true, true} ->
            ...
            case length(NewSignatures) of
                ...
                SigLen when SigLen >= Majority andalso State#state.conf_sent == false ->
                    {State#state{signatures = NewSignatures, conf_sent = true}, [
                        {multicast, t2b({conf, NewSignatures})}
                    ]};
...
handle_message({signature, Address, Signature}, _Index, State) when
    State#state.artifact == undefined
->
    %% don't have artifact yet so cannot verify signatures
        ...
        {NewBBA, {result_and_send, Result, ToSend}} ->
            ...
            case maps:size(BBAResults) == length(State#state.members) of
                true ->
                    ...
                    Txn = blockchain_txn_consensus_group_failure_v1:new(
                        FailedMembers, State#state.height, State#state.delay
                    ),
                    MyAddress = libp2p_crypto:pubkey_to_bin(MyKey),
                    MySignature = blockchain_txn_consensus_group_failure_v1:sign(Txn, SigFun),
                    GoodSignatures = lists:foldl(
                        fun({Address, Signature}, Acc) ->
                            %% only check signatures from members we have not already verified and have not already appeared in this list
                            case
                                {
                            ...
                            of
                                {true, true} ->
                                    ...
                                    [{Address, Signature} | Acc];
                                {true, false} ->
                                    lager:debug("got invalid signature ~p from ~p", [
                                        Signature, Address
                                    ]),
                                    Acc;
                                {false, _} ->
                    ...
                    case length(GoodSignatures) of
                        SigLen when SigLen >= (2 * State#state.f) + 1 ->
                            ...
                            {
                                State#state{
                                    bbas = maps:put(I, NewBBA, State#state.bbas),
                                    bba_results = BBAResults,
                                    signatures = GoodSignatures,
                                    artifact = Txn,
                                    done_called = true
                                },
                                fixup_bba_msgs(ToSend, I)
                            };
                        SigLen when SigLen >= Majority ->
                            {
                                State#state{
                        ...
                        _ ->
                            {
                                ...
                                [
                                    {multicast, t2b({signature, MyAddress, MySignature})}
                                    | fixup_bba_msgs(ToSend, I)
                                ]
                            }
                    end;
                false ->
                    {
                        State#state{
                            bbas = maps:put(I, NewBBA, State#state.bbas), bba_results = BBAResults
                        },
                        fixup_bba_msgs(ToSend, I)
                    }
            end
...
serialize(State) ->
    t2b(State#state{
        privkey = tc_key_share:serialize(State#state.privkey),
        bbas = maps:map(
            fun(_, BBA) ->
