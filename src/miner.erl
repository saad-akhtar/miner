...
hbbft_skip() ->
    case gen_server:call(?MODULE, consensus_group, 60000) of
        undefined -> ok;
        Pid -> ok = libp2p_group_relcast:handle_input(Pid, maybe_skip)
    end.
            ...
            Data =
                case application:get_env(blockchain, gossip_version, 1) of
                    ...
                    2 ->
                        ...
                        {ok, #block_info_v2{hash = Hash}} = blockchain:get_block_info(
                            Height, Chain
                        ),
...
handle_info(
    {blockchain_event, {add_block, Hash, Sync, Ledger}},
    State = #state{
        consensus_group = ConsensusGroup,
                        ...
                        case blockchain_election:has_new_group(Txns) of
                            false ->
                                ...
                                libp2p_group_relcast:handle_input(
                                    ConsensusGroup, {next_round, NextRound, Txns, Sync}
                                ),
                                {ok, ConsensusAddrs} = blockchain_ledger_v1:consensus_members(
                                    Ledger
                                ),
                                F = (length(ConsensusAddrs) - 1) div 3,
                                BBAs = blockchain_utils:bitvector_to_map(
                                    length(ConsensusAddrs),
                                    blockchain_block_v1:bba_completion(Block)
                                ),
                                BBAAddrs = maps:fold(
                                    fun
                                        (K, true, Acc) ->
                                ...
                                case
                                    lists:member(libp2p_crypto:pubkey_to_bin(PubKey), BBAAddrs) orelse
                                        length(BBAAddrs) < (2 * F) + 1
                                of
                                    true ->
                                    ...
                                    false ->
                                        lager:info("jumping to next hbbft round ~p early", [
                                            Round + 1
                                        ]),
                                        ...
                                        erlang:cancel_timer(State#state.late_block_timer),
                                        LateBlockTimeout = application:get_env(
                                            miner, late_block_timeout_seconds, 120
                                        ),
                                        LateTimer = erlang:send_after(
                                            LateBlockTimeout * 1000, self(), late_block_timeout
                                        ),
                                        State#state{
                                            block_timer = Timer, late_block_timer = LateTimer
                                        }
                                end;
                            {true, _, _, _} ->
                                State#state{
...
handle_info({blockchain_event, {new_chain, NC}}, #state{
    blockchain_ref = Ref, swarm_keys = SK, swarm_tid = STid
}) ->
    State1 = #state{
        blockchain = NC,
        blockchain_ref = Ref,
...
select_transactions(Chain, Txns, {ElectionEpoch0, EpochStart0}, BlockHeightCurr, BlockHeightNext) ->
    ...
    {_, ValidTransactions1} = lists:foldl(
        fun
            (_Txn, {done, Acc}) ->
...
set_next_block_timer(State = #state{blockchain = Chain}) ->
    ...
    lager:info("average ~p block times ~p difference ~p", [
        Height, AvgBlockTime, BlockTime - AvgBlockTime
    ]),
    BlockTimeDeviation =
    ...
    LateBlockTimeout = application:get_env(miner, late_block_timeout_seconds, 120),
    LateTimer = erlang:send_after(
        (LateBlockTimeout + NextBlockTime) * 1000, self(), late_block_timeout
    ),
