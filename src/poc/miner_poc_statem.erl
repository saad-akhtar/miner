        ...
        {ok, State, #data{poc_restarts = POCRestarts} = Data} ->
            case is_supported_state(State) of
                ...
                true when Blockchain /= undefined ->
                    ...
                    case
                        miner_util:has_valid_local_capability(?GW_CAPABILITY_POC_CHALLENGER, Ledger)
                    of
                        X when
                            X == ok;
                        ...
                        ->
                            {ok, State,
                                maybe_init_addr_hash(Data#data{
                                    base_dir = BaseDir,
                                    blockchain = Blockchain,
                                    address = Address,
                                    poc_interval = Delay,
                ...
                false ->
                    lager:debug(
                        "Loaded unsupported state ~p, ignoring and defaulting to requesting", [
                            State
                        ]
                    ),
                    {ok, requesting,
...
requesting(enter, _State, Data) ->
    ...
    {keep_state, Data#data{
        poc_restarts = ?POC_RESTARTS, retry = ?CHALLENGE_RETRY, mining_timeout = ?MINING_TIMEOUT
    }};
...
requesting(
    info,
    {blockchain_event, {add_block, BlockHash, Sync, Ledger}} = Msg,
                ...
                false ->
                    case allow_request(BlockHash, Data) of
                        ...
                        true ->
                            ...
                            lager:md([
                                {poc_id,
                                    blockchain_utils:poc_id(libp2p_crypto:pubkey_to_bin(PubKey))}
                            ]),
                            lager:info("request allowed @ ~p", [BlockHash]),
                            Self = self(),
                            TxnRef = make_ref(),
                            ok = blockchain_worker:submit_txn(Txn, fun(Result) ->
                                Self ! {TxnRef, Result}
                            end),
...
mining(
    info,
    {blockchain_event, {add_block, BlockHash, _, PinnedLedger}},
    #data{
        blockchain = _Chain,
        address = Challenger,
    ...
    case find_request(BlockHash, Data) of
        {ok, Block} ->
            ...
            handle_targeting(
                <<Secret/binary, BlockHash/binary, Challenger/binary>>,
                Height,
                PinnedLedger,
                maybe_init_addr_hash(Data#data{
                    mining_timeout = Data#data.poc_interval, request_block_hash = BlockHash
                })
            );
        {error, _Reason} ->
            case MiningTimeout > 0 of
                true ->
                    {keep_state,
                        maybe_init_addr_hash(Data#data{mining_timeout = MiningTimeout - 1})};
                false ->
                    lager:error(
                        "did not see PoC request in last ~p block, giving up on this POC", [
                            Data#data.poc_interval
                        ]
                    ),
                    {next_state, requesting,
                        save_data(maybe_init_addr_hash(Data#data{state = requesting}))}
            end
    end;
...
receiving(
    info,
    {blockchain_event, {add_block, _Hash, _, _}},
    #data{receiving_timeout = 0, responses = Responses} = Data
) ->
    case maps:size(Responses) of
        ...
        _ ->
            ...
            try
                ...
                {next_state, waiting,
                    save_data(maybe_init_addr_hash(Data#data{state = waiting, txn_ref = Ref}))}
            catch
                C:E:S ->
...
receiving(
    ...
    case Address == GatewayWitness of
        false ->
            lager:warning(
                "witness gw: ~p, recv addr: ~p mismatch!",
                [
                                ...
                                case lists:any(Predicate, Witnesses) of
                                    false ->
                                        maps:put(
                                            PacketHash,
                                            lists:keystore(
                                                Address, 1, Witnesses, {Address, Witness}
                                            ),
                                            Responses0
                                        );
                                    true ->
...
receiving(
    cast,
    {receipt, Address, Receipt, PeerAddr},
    #data{responses = Responses0, challengees = Challengees, blockchain = Chain} = Data
) ->
                        ...
                        undefined ->
                            ...
                            case check_addr_hash(PeerAddr, Data) of
                                true when IsFirstChallengee ->
                                    ...
                                    {next_state, requesting,
                                        save_data(Data#data{state = requesting})};
                                true ->
                                    {keep_state, Data};
                                undefined ->
                                ...
                                PeerHash ->
                                    Responses1 = maps:put(
                                        Gateway,
                                        {Address,
                                            blockchain_poc_receipt_v1:addr_hash(Receipt, PeerHash)},
                                        Responses0
                                    ),
                                    {keep_state, save_data(Data#data{responses = Responses1})}
                            end;
...
waiting(
    info, {blockchain_event, {add_block, _BlockHash, _, _}}, #data{receipts_timeout = 0} = Data
) ->
    lager:warning("I have been waiting for ~p blocks abandoning last request", [?RECEIPTS_TIMEOUT]),
    {next_state, requesting,
        save_data(
            maybe_init_addr_hash(Data#data{state = requesting, receipts_timeout = ?RECEIPTS_TIMEOUT})
        )};
waiting(info, {TxnRef, Result}, #data{txn_ref = TxnRef} = Data) ->
    lager:info("PoC receipt submission result ~p", [Result]),
    {keep_state, Data};
waiting(
    info, {blockchain_event, {add_block, BlockHash, _, _}}, #data{receipts_timeout = Timeout} = Data
) ->
...
handle_targeting(_, _, _, Data = #data{retry = Retry}) when
    Retry =< 0
->
...
handle_targeting(Entropy, Height, Ledger, Data) ->
    ...
    case blockchain_ledger_gateway_v2:location(ChallengerGw) of
        undefined ->
            ...
            {next_state, requesting,
                save_data(Data#data{state = requesting, retry = ?CHALLENGE_RETRY})};
        ChallengerLoc ->
            ...
            case blockchain:config(?poc_version, Ledger) of
                {ok, V} when V < 4 ->
                    case
                        blockchain_poc_path:target(Entropy, Ledger, blockchain_swarm:pubkey_bin())
                    of
                        {Target, Gateways} ->
                ...
                {ok, V} when V < 7 ->
                    ...
                    %% Filtered gateways
                    GatewayScores = blockchain_poc_target_v2:filter(
                        GatewayScoreMap, ChallengerAddr, ChallengerLoc, Height, Vars, Ledger
                    ),
                    case blockchain_poc_target_v2:target(Entropy, GatewayScores, Vars) of
                        ...
                        {ok, TargetPubkeyBin} ->
                            ...
                            lager:info("POCVersion: ~p~n", [V]),
                            lager:info("GatewayScores: ~p~n", [GatewayScores]),
                            lager:info("poc_v4 target found ~p, challenging, hash: ~p", [
                                TargetPubkeyBin, Entropy
                            ]),
                            handle_challenging(
                                {Entropy, ignored},
                                TargetPubkeyBin,
                ...
                {ok, V} when V < 8 ->
                    {ok, TargetPubkeyBin} = blockchain_poc_target_v2:target_v2(
                        Entropy, Ledger, Vars
                    ),
                    lager:info("poc_v~p target found ~p, challenging, hash: ~p", [
                        V, TargetPubkeyBin, Entropy
                    ]),
                ...
                {ok, V} ->
                    {ok, {TargetPubkeyBin, TargetRandState}} = blockchain_poc_target_v3:target(
                        ChallengerAddr, Entropy, Ledger, Vars
                    ),
                    lager:info("poc_v~p challenger: ~p, challenger_loc: ~p", [
                        V, libp2p_crypto:bin_to_b58(ChallengerAddr), ChallengerLoc
                    ]),
                    lager:info("poc_v~p target found ~p, challenging, target_rand_state: ~p", [
                        V, libp2p_crypto:bin_to_b58(TargetPubkeyBin), TargetRandState
                    ]),
                    %% NOTE: We pass in the TargetRandState along with the entropy here
                    handle_challenging(
                        {Entropy, TargetRandState},
                        TargetPubkeyBin,
                        ignored,
                        Height,
                        Ledger,
...
handle_challenging(
        ...
        spawn_monitor(fun() ->
            case blockchain:config(?poc_version, Ledger) of
                {ok, V} when V < 4 ->
                    Self !
                        {Attempt,
                            blockchain_poc_path:build(Entropy, Target, Gateways, Height, Ledger)};
                {ok, V} when V < 7 ->
                    Path = blockchain_poc_path_v2:build(
                        Target, Gateways, Time, Entropy, Vars, Ledger
                    ),
                ...
                {ok, V} ->
                    ...
                    Path = blockchain_poc_path_v4:build(
                        Target, TargetRandState, Ledger, Time, Vars
                    ),
                    lager:info("poc_v~p Path: ~p~n", [V, Path]),
                    Self ! {Attempt, {ok, Path}}
            end
    ...
    receive
        ...
        {Attempt, {ok, Path}} ->
            ...
            {Onion, Layers} = blockchain_poc_packet:build(
                OnionKey, IV, OnionList, BlockHash, Ledger
            ),
        ...
        {'DOWN', Ref, process, _Pid, Reason} ->
            lager:error("blockchain_poc_path went down ~p: ~p", [
                Reason, {Entropy, Target, Gateways, Height}
            ]),
            handle_targeting(Entropy, Height, Ledger, Data#data{retry = Retry - 1})
    after Timeout ->
        erlang:demonitor(Ref, [flush]),
        erlang:exit(Pid, kill),
        lager:error("blockchain_poc_path took too long: Entropy ~p Target ~p Height ~p", [
            Entropy, Target, Height
        ]),
            ...
            try erlang:binary_to_term(Binary) of
                Map = #{
                    ...
                    {ok, State, #data{
                        ...
                        mining_timeout = MiningTimeout,
                        retry = Retry,
                        addr_hash_filter = deserialize_addr_hash_filter(
                            maps:get(addr_hash_filter, Map, undefined)
                        ),
                        receipts_timeout = ReceiptTimeout
                    }};
                _ ->
...
maybe_init_addr_hash(
    #data{
        blockchain = Blockchain,
        addr_hash_filter = #addr_hash_filter{
            start = StartHeight, height = Height, byte_size = Bytes, salt = Hash, bloom = Bloom
        }
    } = Data
) ->
    Ledger = blockchain:ledger(Blockchain),
...
-spec update_addr_hash(
    Bloom :: bloom_nif:bloom(), Element :: blockchain_poc_path_element_v1:poc_element()
) -> ok.
...
allow_request(BlockHash, #data{
    ...
    try
            ...
            {ok, GwInfo} ->
                ...
                case
                    blockchain_ledger_gateway_v2:is_valid_capability(
                        GwMode, ?GW_CAPABILITY_POC_CHALLENGER, Ledger
                    )
                of
                    true ->
                        ...
                        ChallengeOK =
                            case blockchain_ledger_gateway_v2:last_poc_challenge(GwInfo) of
                                undefined ->
                                    lager:info(
                                        "got block ~p @ height ~p (never challenged before)", [
                                            BlockHash, Height
                                        ]
                                    ),
                                    true;
        ...
        Class:Err:Stack ->
            lager:warning(
                "error determining if request allowed: ~p:~p ~p",
                [Class, Err, Stack]
            ),
            false
    end.
        ...
        {ok, Block} ->
            ...
            Filter =
                fun(Txn) ->
                    ...
                    OnionKeyHash = crypto:hash(
                        sha256, libp2p_crypto:pubkey_to_bin(OnionCompactKey)
                    ),
...
submit_receipts(
    ...
    Path1 = lists:foldl(
        fun({Challengee, LayerHash}, Acc) ->
            ...
            PerHopMaxWitnesses = blockchain_utils:poc_per_hop_max_witnesses(
        ...
        case blockchain:config(?poc_version, blockchain:ledger(Chain)) of
            {ok, PoCVersion} when PoCVersion >= 10 ->
                blockchain_txn_poc_receipts_v1:new(
                    Challenger, Secret, OnionKeyHash, BlockHash, lists:reverse(Path1)
                );
            _ ->
                blockchain_txn_poc_receipts_v1:new(
                    Challenger, Secret, OnionKeyHash, lists:reverse(Path1)
                )
        end,
    {ok, _, SigFun, _ECDHFun} = blockchain_swarm:keys(),
