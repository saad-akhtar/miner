...
-define(SlowTxns, #{
    blockchain_txn_poc_receipts_v1 => 75,
                            ...
                            case blockchain_txn:absorb(Txn, Chain) of
                                ...
                                Error ->
                                    lager:warning("speculative absorb failed for ~s, error: ~p", [
                                        blockchain_txn:print(Txn), Error
                                    ]),
...
handle_info(
    {Ref, {Res, Height}}, #state{validations = Validations, chain = Chain, group = Group} = State
) when
    is_reference(Ref)
->
...
maybe_start_validation(
    ...
    case Queue of
        ...
        [{From, Txn, Height} | Queue1] ->
            Type = blockchain_txn:type(Txn),
            Timeout = maps:get(
                Type, ?SlowTxns, application:get_env(miner, txn_validation_budget_ms, 10000)
            ),
            {Attempt, V} = start_validation(Txn, Height, From, Timeout, Chain),
...
start_validation(Txn, Height, From, Timeout, Chain) ->
    ...
    {Attempt, #validation{
        timer = TRef, monitor = Ref, txn = Txn, pid = Pid, from = From, height = Height
    }}.
