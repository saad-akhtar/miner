...
create(OldGenesisFile, PubKeyB58, ProofB58, Addrs, N, Curve) ->
    case file:consult(OldGenesisFile) of
        {ok, [Config]} ->
            ...
            OldSecurities =
                case proplists:get_value(securities, Config) of
                    undefined ->
                        [];
                    Securities ->
                        [
                            blockchain_txn_security_coinbase_v1:new(
                                libp2p_crypto:b58_to_bin(proplists:get_value(address, X)),
                                proplists:get_value(token, X)
                            )
                         || X <- Securities
                        ]
                end,
            ...
            OldAccounts =
                case proplists:get_value(accounts, Config) of
                    undefined ->
                        [];
                    Accounts ->
                        [
                            blockchain_txn_coinbase_v1:new(
                                libp2p_crypto:b58_to_bin(proplists:get_value(address, X)),
                                proplists:get_value(balance, X)
                            )
                         || X <- Accounts
                        ]
                end,
            ...
            OldGateways =
                case proplists:get_value(gateways, Config) of
                    undefined ->
                        [];
                    Gateways ->
                        [
                            blockchain_txn_gen_gateway_v1:new(
                                libp2p_crypto:b58_to_bin(proplists:get_value(gateway_address, X)),
            ...
            OldDCs =
                case proplists:get_value(dcs, Config) of
                    undefined ->
                        [];
                    DCs ->
                        [
                            blockchain_txn_dc_coinbase_v1:new(
                                libp2p_crypto:b58_to_bin(proplists:get_value(address, X)),
                                proplists:get_value(dc_balance, X)
                            )
                         || X <- DCs
                        ]
                end,
            ...
            Addresses = [
             ...
             || Addr <- string:split(Addrs, ",", all)
            ],
            miner_consensus_mgr:initial_dkg(
                OldGenesisTransactions ++ [VarTxn1], Addresses, N, Curve
            ),
...
forge(PubKeyB58, ProofB58, Addrs, N, Curve) ->
    ...
    InitialPaymentTransactions = [
        blockchain_txn_coinbase_v1:new(Addr, 500000000)
     || Addr <- Addresses
    ],
    %% Give security tokens to 2 members
    InitialSecurityTransactions = [
        blockchain_txn_security_coinbase_v1:new(Addr, 5000000)
     || Addr <- lists:sublist(Addresses, 2)
    ],
    %% Give DCs to 2 members
    InitialDCs = [
        blockchain_txn_dc_coinbase_v1:new(Addr, 10000000)
     || Addr <- lists:sublist(Addresses, 2)
    ],
    ...
    InitialGatewayTransactions = [
        blockchain_txn_gen_validator_v1:new(Addr, Addr, 10000 * 100000000)
    ...
    case blockchain_worker:blockchain() of
        ...
        Chain ->
            case blockchain:genesis_block(Chain) of
                ...
                {ok, GenesisBlock} ->
                    case
                        (catch file:write_file(Filename, blockchain_block:serialize(GenesisBlock)))
                    of
                        {'EXIT', _} ->
                            usage;
                        ok ->
                            [
                                clique_status:text(
                                    io_lib:format("ok, genesis file written to ~p", [Filename])
                                )
                            ];
                        {error, Reason} ->
                            [
