...
hbbft_queue_cmd() ->
    [
        [
            ...
            [
                {inbound, [
                    {shortname, "i"}, {longname, "inbound"}, {datatype, {enum, [true, false]}}
                ]},
        ...
        false ->
            case proplists:get_value(outbound, Flags) of
                            ...
                            [
                                [{destination, "inbound"}] ++
                                    ...
                                    [
                                        {name, ""},
                                        {count,
                                            integer_to_list(length(maps:get(inbound, Queue, [])))},
                                        {connected, "true"},
                                        {blocked, "false"},
                                        {in_flight, "0"},
                                        {connections, "0"},
                                ...
                                [
                                    [{destination, integer_to_list(K)}] ++
                                        [
                                            {address, maps:get(address, maps:get(K, Workers))}
                                         || lists:keymember(verbose, 1, Flags)
                                        ] ++
                                        [
                                            {name,
                                                element(
                                                    ...
                                                    erl_angry_purple_tiger:animal_name(
                                                        libp2p_crypto:bin_to_b58(
                                                            libp2p_crypto:p2p_to_pubkey_bin(
                                                                maps:get(
                                                                    address, maps:get(K, Workers)
                                                                )
                                                            )
                                                        )
                                                    )
                                            ...
                                            {connected,
                                                atom_to_list(
                                                    (maps:get(connected, maps:get(K, Workers)))
                                                )},
                                            {blocked,
                                                atom_to_list(
                                                    not (maps:get(ready, maps:get(K, Workers)))
                                                )},
                                            {in_flight,
                                                integer_to_list(
                                                    maps:get(in_flight, maps:get(K, Workers))
                                                )},
                                            {connects,
                                                integer_to_list(
                                                    maps:get(connects, maps:get(K, Workers))
                                                )},
                                            {last_take,
                                                atom_to_list(
                                                    maps:get(last_take, maps:get(K, Workers))
                                                )},
                                            {last_ack,
                                                integer_to_list(
                                                    erlang:system_time(second) -
                                                        maps:get(last_ack, maps:get(K, Workers))
                                                )}
                                        ]
                ...
                PeerID when is_integer(PeerID) ->
                    ...
                    [
                        clique_status:table([
                            [{message, lists:flatten(io_lib:format("~p", [Msg]))}]
                         || Msg <- Msgs
                        ])
                    ];
                _ ->
                    ...
                    [
                        [
                            ...
                            {name,
                                element(
                                    2,
                                    erl_angry_purple_tiger:animal_name(libp2p_crypto:bin_to_b58(A))
                                )},
                            {address, libp2p_crypto:pubkey_bin_to_p2p(A)}
                        ]
                     || {Idx, A} <- lists:zip(
                            lists:seq(1, length(ConsensusAddrs)),
                            ConsensusAddrs
                        )
                    ]
                )
    ...
    [
        clique_status:table(
            [
                [
                    {name,
                        element(2, erl_angry_purple_tiger:animal_name(libp2p_crypto:bin_to_b58(A)))}
                ] ++
                    [
                        {address, libp2p_crypto:pubkey_bin_to_p2p(A)}
                     || lists:keymember(verbose, 1, Flags)
                    ...
                    [
                        {bba_completions,
                            io_lib:format("~b/~b", [element(2, maps:get(A, BBATotals)), BlocksSince])},
                        {seen_votes,
                            io_lib:format("~b/~b", [element(2, maps:get(A, SeenTotals)), MaxSeen])},
                        {last_bba,
                            CurrentHeight -
                                max(PostElectionHeight, element(1, maps:get(A, BBATotals)))},
                        {last_seen,
                            CurrentHeight -
                                max(PostElectionHeight, element(1, maps:get(A, SeenTotals)))},
                        {tenure,
                            io_lib:format("~.2f", [
                                element(2, element(2, lists:keyfind(A, 1, GroupWithPenalties)))
                            ])},
                        {penalty,
                            io_lib:format("~.2f", [
                                element(1, element(2, lists:keyfind(A, 1, GroupWithPenalties)))
                            ])}
                    ]
             || A <- ConsensusMembers
            ]
