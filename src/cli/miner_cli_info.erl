...
get_info() ->
    ...
    {ok, #block_info_v2{election_info = {Epoch, _}, height = Height}} = blockchain:head_block_info(
        Chain
    ),
...
info_height(["info", "height"], [], []) ->
    ...
    case SyncHeight == Height of
        ...
        false ->
            [
                clique_status:text([
                    Epoch, "\t\t", integer_to_list(Height), "\t\t", integer_to_list(SyncHeight), "*"
                ])
            ]
    end;
info_height([_, _, _], [], []) ->
...
info_onboarding(["info", "onboarding"], [], Flags) ->
    OnboardingKey =
        case proplists:get_value(key, Flags, undefined) of
            undefined ->
    ...
    case OnboardingKey of
        ...
        OK ->
            case onboarding_info_for_key(OK) of
                ...
                {error, Code} ->
                    error_message(
                        io_lib:format("Onboarding server unexcpetedly returned code ~p.", [Code])
                    )
            end
    end.
...
%% ("MakerData").
%%
-spec onboarding_info_for_key(string()) ->
    {ok, {map(), map()}} | notfound | {error, non_neg_integer()}.
...
info_summary_usage() ->
    [
        ...
        [
            ...
            "  Also returns the last " ++ ?DEFAULT_LOG_HIT_COUNT ++
                " error log entries across 3 categories: Transactions, POCs and Other errors.\n"
                "  By default the latest " ++ ?DEFAULT_LOG_SCAN_RANGE ++
                " log entries are scanned and the most recent " ++ ?DEFAULT_LOG_HIT_COUNT ++
                " hits for each category returned\n\n"
                "Options\n\n"
                "  -e, --error_count <count> "
...
do_info_summary(ErrorCount, ScanRange) ->
    {GeneralInfo, POCErrorInfo, TxnErrorInfo, GenErrorInfo} = get_summary_info(
        ErrorCount, ScanRange
    ),
...
get_log_errors(ErrorCount, ScanRange) ->
    ...
    POCErrors = os:cmd(
        "tail -n" ++ ScanRange ++ " " ++ LogPath ++
            " | sort -rn |"
            "grep -m" ++ ErrorCount ++
            " -E \"error.*miner_poc|miner_poc.*error|error.*blockchain_poc|blockchain_poc.*error\""
    ),
