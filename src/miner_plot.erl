...
handle_info(
    {blockchain_event, {add_block, Hash, _, Ledger}},
    #state{
        chain = Chain,
        height = CurrHeight,
        fd = File,
        ...
        {ok, Block} ->
            case blockchain_block:height(Block) of
                Height when Height > CurrHeight ->
                    ...
                    Avg = ((Time - TAgo) / Period),
                    {Iolist, Stats1} = process_line(
                        {Time, Txns, Epoch, Height, Avg, Size, Interval},
                        Stats
                    ),
...
process_line({0, _Txns, _Epoch, _Height, _Size, _Interval}, Acc) ->
    {[], Acc};
process_line(
    {Time, Txns, {Epoch, EpochStart}, Height, Avg, Size, Int},
    #stats{
        times = Times0,
        last_rewards = Last,
        tlens = TLens1
            ...
            _else ->
                case extract_rewards(Txns) of
                    {ok, Type, Rwds} ->
                        ...
                        {integer_to_list(trunc((Bones / Ideal) * 100)), Time};
                    _ ->
                        {"", Last}
                end
        end,
