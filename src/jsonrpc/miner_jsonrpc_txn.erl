...
handle_rpc(<<"txn_add_gateway">>, #{<<"owner">> := OwnerB58} = Params) ->
    try
        Payer =
            case maps:get(payer, Params, undefined) of
                undefined -> undefined;
        ...
        T:E:St ->
            lager:error(
                "Couldn't do add gateway via JSONRPC because: ~p ~p: ~p",
                [T, E, St]
            ),
...
handle_rpc(<<"txn_assert_location">>, #{<<"owner">> := OwnerB58} = Params) ->
    try
        ...
        H3String =
            case parse_location(Params) of
                {error, _} = Err -> throw(Err);
                {ok, S} -> S
            end,
        Nonce = maps:get(nonce, Params, 1),
        {ok, Bin} = blockchain:assert_loc_txn(
            H3String,
            ?B58_TO_BIN(OwnerB58),
            Payer,
            Nonce
        ...
        T:E:St ->
            lager:error(
                "Couldn't complete assert location JSONRPC because ~p ~p: ~p",
                [T, E, St]
            ),
            Error = io_lib:format("~p", [E]),
            ?jsonrpc_error({error, Error})
...
parse_location(_Other) ->
    {error, no_valid_location_found}.
